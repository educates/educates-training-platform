...
To delete text that has been previously selected using ``editor:select-matching-text`` or ``editor:select-lines-in-range``, use:
...
This is a two-step process: first select the text, then delete it. If there is no current selection, the action is a no-op.

To insert lines immediately before the current text selection, use:
...
To append lines immediately after the current text selection, use:
...
Both ``editor:insert-lines-before-selection`` and ``editor:append-lines-after-selection`` are two-step operations requiring text to be selected first using ``editor:select-matching-text`` or ``editor:select-lines-in-range``.
...
To prepend lines to the beginning of a file, use:
...
As with ``editor:append-lines-to-file``, if the file doesn't exist it will be created.
...
Note that ``editor:insert-lines-after-line`` is deprecated but still works. Use ``editor:append-lines-after-line`` instead, which is consistent with the naming convention where "insert" means before and "append" means after.
...
To insert lines before matching a line containing a specified string, use:
